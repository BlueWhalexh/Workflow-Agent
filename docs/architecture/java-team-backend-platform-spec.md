...
当前实现状态：J7A 已实现 Spring Security local-dev baseline。请求会通过 dev-header filter 写入 `SecurityContext`，支持 `X-Dev-User-Id`、`X-Dev-Team-Id`、`X-Dev-Display-Name` 覆盖；没有 dev header 时回退到配置里的 dev principal。workspace membership 是 role truth source，服务层按 role hierarchy 执行 guard，并为敏感动作追加 audit metadata。J31A 已实现 production-like identity fail-closed baseline：当 active profile 包含 `prod` 或 `production` 时，dev identity 被禁用，`X-Dev-*` header 不会被信任，未认证请求不会回退到配置 dev principal，而是返回 `java-backend-api.v1` 的 `AUTHENTICATION_REQUIRED` 401 error envelope。J7A/J31A 仍不是完整 OIDC/OAuth、SSO、session management、邀请流、全量 user/team directory CRUD 或生产 RBAC 同步。
