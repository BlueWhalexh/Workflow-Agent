...
- 架构上保留未来 remote runner 扩展位；J30A 已实现 workspace-scoped runner 注册、心跳和租约元数据 baseline，J41A 已实现 `remoteRunnerRef` 对已注册 online runner 的显式派发 baseline，但仍不实现 artifact upload、remote cancellation、runner identity、mTLS、runner-scoped secret distribution 或 multi-node scheduler。
...
- `runner`
  ...
  - J41A 后 `POST /v1/workspaces/{workspaceId}/agent-runs` 可携带 `remoteRunnerRef` 显式派发到同 workspace 已注册且 online、具备 `agent-backend-response.v1` capability 的 runner endpoint。
  - 不分发 runner token、provider secret、runner-scoped secret 或生产 runner identity。
- `providersecret`
  ...
  - J29A 后，`secret://...` metadata 可由后端 resolver SPI 解析为内存 secret，并通过 `AgentWorkerSecretInjection` 作为 per-run env 注入给支持该能力的本地 worker；J33A 后，配置了 `my-workflow.backend.provider-secrets.file-root` 时，`file://...` metadata 可解析为该根目录内的本地文件 secret；J42A 后，配置了 `my-workflow.backend.provider-secrets.http-resolver-uri` 时，`secret://...` 可通过外部 HTTP secret manager resolver 解析。`AgentWorkerRequest` 只包含 `apiKeyEnvName`，不包含 `apiKeySecretRef`、file path、secret manager auth token 或 raw secret。该阶段不实现 vendor KMS/Keychain adapter、secret rotation、public secret registration 或 remote runner secret distribution。
- `audit`
...
- Phase J41A `POST /v1/workspaces/{workspaceId}/agent-runs` 可选 `remoteRunnerRef`；存在时 Java 从同 workspace runner registry 解析 online runner，并通过 `RemoteHttpAgentWorker` 派发到其 endpoint。该请求字段只表示显式调度目标，public run response 不返回 runner endpoint、runner token、signature secret、provider secret 或 Authorization material。
...
当前实现状态：J7A 已实现 Spring Security local-dev baseline。请求会通过 dev-header filter 写入 `SecurityContext`，支持 `X-Dev-User-Id`、`X-Dev-Team-Id`、`X-Dev-Display-Name` 覆盖；没有 dev header 时回退到配置里的 dev principal。workspace membership 是 role truth source，服务层按 role hierarchy 执行 guard，并为敏感动作追加 audit metadata。J31A 已实现 production-like identity fail-closed baseline：当 active profile 包含 `prod` 或 `production` 时，dev identity 被禁用，`X-Dev-*` header 不会被信任，未认证请求不会回退到配置 dev principal，而是返回 `java-backend-api.v1` 的 `AUTHENTICATION_REQUIRED` 401 error envelope。J32A 已实现 OIDC-ready bearer verifier adapter baseline：`BearerTokenAuthenticationFilter` 读取 `Authorization: Bearer ...`，通过可注入 `BearerTokenVerifier` SPI 把 opaque token 映射为 `BackendPrincipal` 并写入 `SecurityContext`；无 verifier 或 verifier 拒绝时继续 fail closed，不回显 token。J36A 已实现 configurable OIDC/JWKS JWT verifier baseline：配置 `my-workflow.backend.oidc.jwks-uri` 后，后端可用 Spring Security `NimbusJwtDecoder` 校验 JWKS 签名 JWT，并按 `issuer`、`audience`、`user-id-claim`、`team-id-claim`、`display-name-claim` 显式映射到 `BackendPrincipal`；签名、issuer、audience 或 required claim 不通过时返回 empty principal。J37A 已实现 OIDC issuer discovery + redacted auth diagnostics baseline：配置 `my-workflow.backend.oidc.issuer-uri` 后可从 discovery document 解析 `jwks_uri`，拒绝 `issuer-uri`/`jwks-uri` 混配和缺失 `jwks_uri` 的 discovery payload，并通过 `GET /v1/ops/auth-config` 只暴露 mode、configured booleans 和 claim names，不返回 raw issuer URI、JWKS URI、token、secret 或 Authorization material。J39A 已实现 OAuth token introspection bearer verifier baseline：配置 `my-workflow.backend.oauth.introspection-uri` 后可把 bearer token POST 到 introspection endpoint，要求 `active=true`，按显式字段映射 `BackendPrincipal`，并与 OIDC issuer/JWKS 配置互斥；可选 client auth 只读取环境变量名指向的 secret，不把 raw secret 写入配置或响应。J34A 已实现 local team directory lifecycle baseline：active `TEAM_ADMIN` 可 upsert/disable 当前 team 的 backend-known member metadata，普通 member 管理目录返回 `TEAM_FORBIDDEN`，disabled member 不能保留或重新获得 workspace access。J35A 已实现 local team invite onboarding baseline：active `TEAM_ADMIN` 可创建/列表/撤销当前 team invite，受邀用户只能接受自己的 pending invite，accept 后才创建 active team membership；invite response 只包含 metadata，不包含 token、secret、workspace path 或 server storage ref。J40A 已实现 external directory snapshot sync baseline：active `TEAM_ADMIN` 可通过 `POST /v1/teams/{teamId}/directory-sync` 导入结构化外部目录成员快照，并可显式 `disableMissing` 停用缺失的 backend-known active members，同时保护当前管理员不被同步请求停用；请求拒绝 `token`、`authorization`、`Authorization`、`secret`、`rawPayload`，响应只返回同步计数和成员公开 metadata。J7A/J31A/J32A/J34A/J35A/J36A/J37A/J39A/J40A 仍不是完整 OAuth login flow、SSO、session management、refresh token、真实 email invite link、SCIM/LDAP/IdP connector、目录同步调度器、目录凭证存储、team-scoped audit schema 或生产 RBAC 同步。
...
J14A 已实现 backend-known team member listing baseline。`GET /v1/teams/{teamId}/members` 只允许当前 principal 查询自己的 team，跨 team 请求返回 `TEAM_FORBIDDEN`。J34A 后响应返回 `teamId`、`userId`、`displayName`、`role`、`status`；workspace 创建会记录 owner 为 `TEAM_ADMIN`，workspace grant 会记录目标用户为 `TEAM_MEMBER`；workspace member removal 只撤销 workspace access，不删除 team membership。J34A 同时新增 `PUT /v1/teams/{teamId}/members/{userId}` 和 `POST /v1/teams/{teamId}/members/{userId}/disable`，仅 active `TEAM_ADMIN` 可管理当前 team 的 backend-known member metadata；disabled member 不能保留或被 grant 到 workspace。J35A 新增 `POST /v1/teams/{teamId}/invites`、`GET /v1/teams/{teamId}/invites`、`POST /v1/teams/{teamId}/invites/{inviteId}/accept` 和 `POST /v1/teams/{teamId}/invites/{inviteId}/revoke`，用本地 DB/in-memory metadata 表达 pending/accepted/revoked invite 状态；accept 才会 upsert active team membership。J40A 新增 `POST /v1/teams/{teamId}/directory-sync`，仅 active `TEAM_ADMIN` 可导入结构化外部目录快照；`disableMissing=true` 会停用快照缺失的 backend-known active members，但不会停用当前管理员；raw directory payload/token/secret aliases 会被拒绝且不回显。J14A/J34A/J35A/J40A 不实现 email、真实 invite token/link、全局 user directory、跨团队 discovery、team CRUD、SCIM/LDAP/IdP connector、目录同步调度器、目录凭证存储、team-scoped audit schema 或生产组织角色同步。
...
- job lease；
- registered-runner explicit dispatch；
- artifact upload；
...
当前实现状态：J7A 已实现 local-dev SecurityContext principal、workspace role guard 和 append-only audit baseline。已覆盖 `GET /v1/me` dev header override/default fallback、viewer 只读边界、editor/owner run/approval 写权限、workspace/run/artifact/approval 关键动作 audit metadata，以及 JDBC audit persistence。J31A 后续补充 production-like identity fail-closed baseline：`prod` / `production` profile 下禁用 dev header 和 dev principal fallback，未认证 principal access 返回 `AUTHENTICATION_REQUIRED`。J32A 后续补充 bearer verifier adapter baseline：生产类 profile 可通过注入式 `BearerTokenVerifier` 验证 `Authorization: Bearer ...` 并建立 `BackendPrincipal`，同时保持 dev header 禁用和 invalid bearer fail-closed。J36A 后续补充 configurable OIDC/JWKS JWT verifier baseline：配置 JWKS URI 后可校验签名 JWT、issuer/audience 和 required claims，并通过显式 claim mapping 建立 `BackendPrincipal`。J37A 后续补充 issuer discovery 与 redacted ops diagnostics baseline：配置 issuer URI 后可解析 discovery `jwks_uri`，并提供不泄露 raw URI/token/secret 的 auth config 自检。J39A 后续补充 OAuth token introspection bearer verifier baseline：配置 introspection endpoint 后可验证 opaque bearer token，并把 active response 的配置字段映射为 `BackendPrincipal`；client auth secret 只通过 env-name lookup 注入 verifier。J34A 后续补充 local team directory lifecycle baseline：active `TEAM_ADMIN` 可 upsert/disable 当前 team member metadata，disabled member 不能保留或被授予 workspace access。J35A 后续补充 local team invite onboarding baseline：active `TEAM_ADMIN` 可创建/列表/撤销 invite，受邀用户可接受自己的 pending invite 并成为 active team member。J40A 后续补充 external directory snapshot sync baseline：active `TEAM_ADMIN` 可导入结构化外部目录成员快照，显式停用缺失成员，并拒绝 raw payload/token/secret alias。J7A/J31A/J32A/J34A/J35A/J36A/J37A/J39A/J40A 不包含完整 OAuth login flow、SCIM/LDAP/IdP connector、目录同步调度器、目录凭证存储、team-scoped audit schema、真实 email/link invite flow、remote runner authorization、SSE/WebSocket authorization 或 provider secret policy。J11A 后续补充 owner-only audit listing API baseline，J12A 后续补充 member removal，J15A 后续补充 owner transfer。
...
当前实现状态：J9A 已实现 provider runtime reference policy baseline。public run API 接受 `providerRuntimeRef`，拒绝 raw `providerRuntime` object 和 token value；Java 只向 worker 传安全 provider metadata 和 `apiKeyEnvName`。J23A 后续补充 provider runtime raw secret config guard：configured refs 会拒绝非空 `api-key`、`token` 或 `authorization`。J23B 后续补充 `api-key-env-name` format guard：只有 env-var-style identifier 会被转成 worker metadata。J24A 后续补充 provider credential metadata schema：DB 有 `provider_credentials` 元数据表和 `api_key_secret_ref`，但没有 raw secret value 列。J24B 后续补充 provider credential metadata repository：后端可按 team/workspace scope 读写 ACTIVE credential metadata。J24C 后续补充 provider credential service scope guard：内部 service 必须先通过 workspace editor guard，再使用 workspace 的 authoritative team/workspace scope 解析 credential metadata。J24D 后续补充 provider credential runtime descriptor：已授权 metadata 可转为 secret-safe descriptor，且 plain secret-looking ref/unsupported provider 会 fail closed。J24E 已补充 agent-run DB-backed credential ref wiring：`providerRuntimeRef = "credential.<credentialRef>"` 会解析 DB credential metadata，并把 `env://NAME` 映射成 worker `apiKeyEnvName`，避免把 `apiKeySecretRef` 或 raw secret 传入 worker request。J25A 已补充 workspace-scoped provider credential public metadata API：owner 可以 upsert/list env-backed metadata，public response 只返回 credential ref、workspace scope、provider/model/baseUrl/status，不返回 env name 或 secret reference。J26A 已补充 workspace-scoped provider credential disable lifecycle guard：owner 可以禁用 metadata，disabled ref 不会被 run path 解析，disable audit 不包含 env name 或 secret reference。J29A 已补充 provider secret injection baseline：`secret://...` refs 可由后端 `ProviderSecretResolver` SPI 解析，并以 per-run env 注入给支持 secret injection 的 worker；`AgentWorkerRequest` 仍只包含 `apiKeyEnvName`，不会序列化 raw secret 或 secret ref。J33A 已补充 local file secret resolver baseline：配置 file root 后，`file://...` refs 可从 root-relative 文件解析，并通过同一 secret injection 路径进入本地 worker。J42A 已补充 HTTP external secret manager resolver baseline：`secret://...` refs 可通过配置的 HTTP endpoint 解析，secret manager auth token 只来自 env-name lookup，raw secret 只进入 in-memory worker env injection。J9A/J23A/J23B/J24A/J24B/J24C/J24D/J24E/J25A/J26A/J29A/J33A/J42A 不包含 vendor KMS/Vault/Keychain SDK、secret rotation、真实 provider 调用、team-scoped credential API 或 remote runner secret distribution。
...
当前实现状态：J14A 已实现 backend-known team member listing baseline。`GET /v1/teams/{teamId}/members` 返回当前 team 的 backend-known members；J34A 后字段为 `teamId`、`userId`、`displayName`、`role`、`status`。当前 principal 只能查询自己的 team；跨 team 查询返回 `TEAM_FORBIDDEN`。Team membership 独立于 workspace membership：删除 workspace 成员不会删除 team membership。J34A 已实现 local lifecycle baseline：active `TEAM_ADMIN` 可通过 `PUT /v1/teams/{teamId}/members/{userId}` upsert member metadata，可通过 `POST /v1/teams/{teamId}/members/{userId}/disable` 禁用成员；disabled member 不会保留旧 workspace access，也不会被 workspace grant 重新加入 workspace。J35A 已实现 local invite onboarding baseline：invite 以 `PENDING`、`ACCEPTED`、`REVOKED` metadata 保存，public response 不含 raw token/secret/link，accept 后才创建 active team membership。J14A/J34A/J35A 不包含完整用户资料目录、团队 CRUD、真实邮件邀请、跨团队 discovery、外部目录同步或生产 OIDC team role 同步。
...
当前实现状态：J29A 已实现本地 provider secret injection baseline。JDBC credential metadata 中的 `secret://...` ref 可由后端 `ProviderSecretResolver` SPI 解析成内存 secret value；`AgentRunService` 为该 secret 生成固定 worker env name `PROVIDER_CREDENTIAL_API_KEY`，并通过 `AgentWorkerSecretInjection` 传给支持该能力的 worker。J33A 已实现 local file secret resolver baseline：配置 `my-workflow.backend.provider-secrets.file-root` 后，`FileProviderSecretResolver` 只接受 root-relative `file://...` refs，并拒绝路径穿越、绝对路径、目录、缺失文件和空白 secret。J42A 已实现 HTTP external secret manager resolver baseline：配置 `my-workflow.backend.provider-secrets.http-resolver-uri` 后，`HttpProviderSecretResolver` 只处理 `secret://...` refs，POST `provider-secret-resolve-request.v1` ref-only JSON，可选 bearer auth 只从 configured env var 读取，并只接受 `provider-secret-resolve-response.v1` 的非空小型 `secretValue`。`LocalTsAgentWorker` 只把 secret 注入子进程环境，仍只把 `AgentWorkerRequest` JSON 写入 stdin；worker request、provider runtime map、run artifact 和 public response 不包含 raw secret、file path、secret manager auth token 或 `apiKeySecretRef`。默认 worker 不支持非空 secret injection，避免 remote HTTP worker 在没有专门 secret distribution 设计时接收 raw secret。J29A/J33A/J42A 不提供 vendor KMS/Vault/Keychain SDK、secret rotation、public secret registration API、remote runner secret distribution 或真实 provider 调用。

### Phase J30: Remote Runner Registry And Lease Baseline

当前实现状态：J30A 已实现 workspace-scoped remote runner registry / heartbeat / lease baseline。JDBC profile 下新增 `remote_runners` schema；workspace owner 可以通过 `PUT /v1/workspaces/{workspaceId}/remote-runners/{runnerRef}` 注册 runner endpoint/capabilities，通过 `POST .../heartbeat` 记录在线状态，通过 `POST .../lease` claim 有 TTL 的 exclusive lease，并通过 `GET .../remote-runners` 列出当前 workspace runner metadata。viewer 调用返回 `WORKSPACE_FORBIDDEN`；endpoint URL 拒绝 userinfo credentials；public response 不返回 runner token、signature secret、provider token、Authorization material 或 raw secret alias；注册、heartbeat 和 lease 会写入 workspace audit metadata。J41A 已把 registry 接入显式 run dispatch：创建 run 时携带 `remoteRunnerRef` 会要求同 workspace runner 已 online 且具备 `agent-backend-response.v1` capability，并通过 existing remote HTTP worker contract 执行。J30A/J41A 不实现 remote artifact upload、remote cancellation、runner identity/auth token、mTLS、multi-node scheduler、runner-scoped credential access 或 remote runner secret distribution。
...
- Contract：
  ...
  - `GET /v1/ops/integration-contract` 暴露前端/runtime 接入所需 endpoint 清单、公共 envelope schema、SSE replay 语义和 remaining-gap readiness flags；
