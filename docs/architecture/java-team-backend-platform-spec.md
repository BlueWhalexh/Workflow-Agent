...
- `providersecret`
  ...
  - J29A 后，`secret://...` metadata 可由后端 resolver SPI 解析为内存 secret，并通过 `AgentWorkerSecretInjection` 作为 per-run env 注入给支持该能力的本地 worker；`AgentWorkerRequest` 只包含 `apiKeyEnvName`，不包含 `apiKeySecretRef` 或 raw secret。该阶段不实现外部 secret manager/KMS、Keychain/file adapter 或 remote runner secret distribution。
- `audit`
